...
# h_hist = np.average(img,axis=1).tolist()
        # peaks = self._get_peaks(h_hist)
        # if len(peaks) < 3:
        #     return -1

        # peaks_sorted = peaks.copy()
        # mid = len(h_hist)/2
        # peaks_sorted.sort(key=lambda v: abs((v - mid)))
        # p1, p2 = peaks_sorted[0], peaks_sorted[1]
        # mid_max = p1 if h_hist[p1] > h_hist[p2] else p2
        # m = max(peaks, key=lambda i: h_hist[i])

        # mid_pos = mid_max/len(h_hist)
        # if mid_pos < .25 or mid_pos > .65 or (h_hist[mid_max]/h_hist[m]) < .93:
        #     return 1
        # return 0
